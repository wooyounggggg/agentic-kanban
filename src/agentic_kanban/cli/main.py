"""Click-based CLI for agentic-kanban."""
...
from agentic_kanban.models.config import BoardConfig, DEFAULT_STATUSES
...
def cli(ctx: click.Context) -> None:
    """agentic-kanban: TUI Kanban Board for Git worktree-based parallel development."""
    if ctx.invoked_subcommand is None:
        try:
            from agentic_kanban.ui.app import WtBoardApp
            app = WtBoardApp()
            app.run()
        except ImportError:
            console.print(
...
def migrate() -> None:
    ...
    if board_dir is None:
        console.print("[red]Run [bold]agentic-kanban init[/bold] first.[/red]")
        sys.exit(1)

    from agentic_kanban.store.migration import migrate_from_wt_state
